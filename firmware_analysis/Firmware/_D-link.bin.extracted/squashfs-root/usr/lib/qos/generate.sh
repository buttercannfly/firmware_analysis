...
start_cg() {
	...
	local sizerules
	enum_classes "$cg"
	add_rules iptrules "$ctrules" "iptables -w -t mangle -A qos_${cg}_ct"
	...
	for class in $classes; do
		...
		[ -z "$maxsize" -o -z "$mark" ] || {
			add_insmod xt_length
			append pktrules "iptables -w -t mangle -A qos_${cg} -m mark --mark $mark/0x0f -m length --length $maxsize: -j MARK --set-mark 0/0xff" "$N"
		}
	done
	add_rules pktrules "$rules" "iptables -w -t mangle -A qos_${cg}"
	for iface in $INTERFACES; do
		config_get classgroup "$iface" classgroup
		config_get device "$iface" device
		config_get ifbdev "$iface" ifbdev
		...
		download="${download:-${halfduplex:+$upload}}"
		append up "iptables -w -t mangle -A OUTPUT -o $device -j qos_${cg}" "$N"
		append up "iptables -w -t mangle -A FORWARD -o $device -j qos_${cg}" "$N"
	done
	cat <<EOF
$INSMOD
iptables -w -t mangle -N qos_${cg} >&- 2>&-
iptables -w -t mangle -N qos_${cg}_ct >&- 2>&-
${iptrules:+${iptrules}${N}iptables -w -t mangle -A qos_${cg}_ct -j CONNMARK --save-mark --mask 0xff}
iptables -w -t mangle -A qos_${cg} -j CONNMARK --restore-mark --mask 0x0f
iptables -w -t mangle -A qos_${cg} -m mark --mark 0/0x0f -j qos_${cg}_ct
$pktrules
${iptrules:+${iptrules}${N}iptables -w -t mangle -A qos_${cg} -j CONNMARK --save-mark --mask 0xf0}
$up$N${down:+${down}$N}
EOF
	unset INSMOD
}
...
stop_firewall() {
	...
	iptables -w -t mangle -S |
		...
		sed -n -e 's/^./iptables -w -t mangle &/p'
}
