...
ipconflict(){
       ...
       guest_ip=$(uci get network.guest.ipaddr)
       guest_mask=$(uci get network.guest.netmask)
       lan_start=$(uci get dhcp.lan.start)
       
       local frist_lanbackups="192.168.0.1"
       local second_lanbackups="192.168.100.1"
       local frist_guestbackups="192.168.7.1"
       local second_guestbackups="192.168.107.1"
       local len

	#uci show network >/dev/console

	if [ -f "/var/set_confick" ];then
  		rm -rf /var/set_confick 
        fi       

       len=`echo $wan_mask|awk '{print length($0)}'`
       echo "$lan_ip $lan_mask $wan_ip $wan_mask $lan_start $len" >/dev/console 
       
       if [ "$len" = "1" -o "$len" = "2" ] ;then
               echo "ip/mask=$(ipcalc -n $wan_ip/$wan_mask)">/dev/console
               [ "$(ipcalc -n $lan_ip -m $lan_mask|grep NETWORK)" = "$(ipcalc -n $wan_ip/$wan_mask)" ] && {
                       [ "$(ipcalc -n $lan_ip -m $lan_mask|grep NETWORK)" != "$(ipcalc -n $frist_lanbackups -m $lan_mask|grep NETWORK)" ] && echo "0 $frist_lanbackups" >/var/set_confick || echo "0 $second_lanbackups" >/var/set_confick

                       #lan_ip=$(uci get network.lan.ipaddr)
		       #uci set dhcp.@domain[0].ip="$lan_ip"
                       #uci commit
                       #reboot  
               }
       else
               echo "ip/mask=$(ipcalc -n $lan_ip -m $lan_mask|grep NETWORK)">/dev/console
               [ "$(ipcalc -n $lan_ip -m $lan_mask|grep NETWORK)" = "$(ipcalc -n $wan_ip -m $wan_mask|grep NETWORK)" ] && {
                       [ "$(ipcalc -n $lan_ip -m $lan_mask|grep NETWORK)" != "$(ipcalc -n $frist_lanbackups -m $lan_mask|grep NETWORK)" ] && echo "0 $frist_lanbackups" >/var/set_confick || echo "0 $second_lanbackups" >/var/set_confick
       ...
       if [ "$len" = "1" -o "$len" = "2" ] ;then
               echo "ip/mask=$(ipcalc -n $wan_ip/$wan_mask)">/dev/console
               [ "$(ipcalc -n $guest_ip -m $guest_mask|grep NETWORK)" = "$(ipcalc -n $wan_ip/$wan_mask)" ] && {
                       [ "$(ipcalc -n $guest_ip -m $guest_mask|grep NETWORK)" != "$(ipcalc -n $frist_guestbackups -m $guest_mask|grep NETWORK)" ] && echo "1 $frist_guestbackups" >/var/set_confick || echo "1 $second_guestbackups" >/var/set_confick

                       #guest_ip=$(uci get network.guest.ipaddr)
                       #uci set dhcp.@domain[1].ip="$guest_ip"
                       #uci commit
                       #reboot
               }
       else
               echo "ip/mask=$(ipcalc -n $guest_ip -m $guest_mask|grep NETWORK)">/dev/console
               [ "$(ipcalc -n $guest_ip -m $guest_mask|grep NETWORK)" = "$(ipcalc -n $wan_ip -m $wan_mask|grep NETWORK)" ] && {
                       [ "$(ipcalc -n $guest_ip -m $guest_mask|grep NETWORK)" != "$(ipcalc -n $frist_guestbackups -m $guest_mask|grep NETWORK)" ] && echo "1 $frist_guestbackups" > /var/set_confick || echo "1 $second_guestbackups" > /var/set_confick
...
_proto_push_ipv4_addr() {
	...
	[ -n "$address" ] && [ -n "$mask" ] && ipconflict "$address" "$mask" 
	json_add_object ""
