...
ppp_generic_init_config() {
	...
	proto_config_add_string pppoetype
...
ppp_generic_setup() {
	...
	json_get_vars demand keepalive pppoetype username password pppd_options pppname
        if [ -z "$pppoetype" ]; then
                pppoetype=normal
        fi;
	enc_username=$(skynet $username $pppoetype)
	...
	proto_run_command "$config" /usr/sbin/pppd \
		...
		usepeerdns \
		$demand maxfail 1 \
		${username:+user "$enc_username" password "$password"} \
...
ppp_generic_teardown() {
	...
	uci set network.$interface.pppoestatus=$ERROR 
	uci commit network
	case "$ERROR" in
		11|19)
